    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-gradient-to-br from-black via-pink-950/50 to-black px-4 text-white">
        <div className="w-full max-w-md rounded-3xl border border-pink-500/40 bg-black/80 p-8 text-center shadow-2xl shadow-pink-500/20">
          ...
          {loadingCities && (
            <p className="mb-3 text-sm text-pink-300">Recherche de ta ville...</p>
          )}
          ...
          <p className="mt-4 text-xs text-gray-500">
            🔞 18+ uniquement • Modération active
          </p>
        </div>
      </main>
    );
    ...
    <main className="flex min-h-screen bg-black text-white">
      ...
      <section className="flex flex-1 flex-col">
        <header className="flex items-center justify-between border-b border-pink-500/20 p-4">
          <div>
            ...
            <p className="text-sm text-gray-400">
              Connecté : {username} • {city} • Département {myDepartment}
            </p>
          </div>
              ...
              <div className="mb-1 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {msg.city || "Ville inconnue"} • dep {msg.postal_code?.slice(0, 2) || "--"}
                  </p>
                </div>
              </div>
              <p>{msg.text}</p>
        ...
        <div className="flex gap-2 border-t border-pink-500/20 p-4">
          ...
          <button
            onClick={sendMessage}
            className="rounded-full bg-pink-500 px-6 py-3 font-black text-black transition hover:bg-pink-400"
          >
            ➤
          </button>
        </div>
      ...
      <aside className="hidden w-80 border-l border-pink-500/20 bg-black/60 p-4 md:block">
        ...
        <div className="mb-3 flex gap-2">
          <button
            ...
            className={`flex-1 rounded-full py-2 text-sm transition ${
              genderFilter === "tous" ? "bg-pink-500 text-black" : "bg-white/10"
            }`}
          >
            Tous
          </button>
          <button
            ...
            className={`flex-1 rounded-full py-2 text-sm transition ${
              genderFilter === "homme" ? "bg-blue-500 text-white" : "bg-white/10"
            }`}
          >
            ♂
          </button>
          <button
            ...
            className={`flex-1 rounded-full py-2 text-sm transition ${
              genderFilter === "femme" ? "bg-pink-500 text-black" : "bg-white/10"
            }`}
          >
            ♀
          </button>
        ...
        <div className="mb-4 flex gap-2">
          <button
            onClick={() => setLocationFilter("departement")}
            className={`flex-1 rounded-full py-2 text-xs transition ${
              locationFilter === "departement"
                ? "bg-pink-500 text-black"
                : "bg-white/10"
            }`}
          >
          ...
          <button
            onClick={() => setLocationFilter("ville")}
            className={`flex-1 rounded-full py-2 text-xs transition ${
              locationFilter === "ville" ? "bg-pink-500 text-black" : "bg-white/10"
            }`}
          >
            Ma ville
          </button>
        ...
        <div className="space-y-3">
          ...
          {filteredUsers.map((user, i) => (
            <div
              key={`${user.username}-${i}`}
              className="flex items-center gap-3 rounded-2xl bg-white/10 p-3 transition hover:bg-pink-500/20"
            >
              <img
                src={user.avatar}
                alt="avatar"
              ...
              <div>
                <p className="font-bold">
                  {user.gender === "femme" ? "♀" : "♂"} {user.username}
                </p>
                <p className="text-xs text-gray-400">
                  {user.age} ans • {user.city} • dep {user.postalCode?.slice(0, 2)}
                </p>
